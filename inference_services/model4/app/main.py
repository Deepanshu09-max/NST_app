# inference_services/model4/app/main.py
...
# Ensure model4_inference is importable
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from model4_inference import generate

# === ADDED: define the target image size ===
image_size = 512  # adjust this if your model expects a different resolution

@app.get("/")
def read_root():
    return {"message": "Inference Service Model 4"}
...
def resolve_ckpt_prefix(prefix: str) -> str:
    """
    Given a prefix like "/persistent_storage/models/model4/XYZ",
    validate that .data-, .index, and .meta shards all exist.
    """
...
async def infer(
    ...
    # Resolve the shared prefix into actual checkpoint files
    ...
    try:
        ...
        # Run the style-transfer generate() call
        generate(
        ...
        if not os.path.exists(temp_output):
            # fallback: pick any file matching the prefix
            candidates = [
        ...
        # re-raise our 400 errors from resolve_ckpt_prefix
        raise
